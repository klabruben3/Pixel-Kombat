...
int main(){
    ...
    Texture food_atlas;
    if (!food_atlas.loadFromFile("C:\\Users\\klabr\\Desktop\\Projects\\2d project\\Pixel Kombat\\genesis\\assets\\food.png")) return -1;
        ...
        for(vector<brick> &row : wall){
            for(brick &col : row){
                ...
                if(col.is_food){
                    ...
                    if(abs(yDifference) < tolerance){ // meaning if pac is at the same row as the block
                        if(xDifference > 0){ // 👈 AKA pac is on the right of the block
                        ...
                        if(xDifference < 0){ // 👈 AKA pac is on the left of the block
                            if(xDifference >= -FRAME_SIZE){
                                runR = true;                        
                            }
                            if(xDifference > -FRAME_SIZE){
                    ...
                    if(abs(xDifference) < tolerance){ // Meaning if pac is at the same column as the block
                        if(yDifference > 0){ // 👈aka when pacman is below the food tile center
                        ...
                        if(yDifference < 0){ // 👈 when pacman is pn top of the food tile center
                            if(yDifference >= -FRAME_SIZE){
                                runD = true;
                            }
                            if(yDifference > -FRAME_SIZE){
